...
namespace Dbg {
   ...
   public:
      enum ECounter {
         kBoundCovChanged,
         kFullTransportJacobianChanged,
         kFreeTransportJacobianChanged,
         kFreeToPathDeritivesChanged,
         kBoundToFreeJacobianChanged,
         kSampling,
         kNotSampling,
         kSamplingSteppingHelper,
         kNotSamplingSteppingHelper,
         kInvalidCovPos,
         kInvalidFreeCovPos,
         kInvalidCovNeg,
         kInvalidFreeCovNeg,
         kInvalidCovPosSteppingHelper,
         kInvalidFreeCovPosSteppingHelper,
         kInvalidCovNegSteppingHelper,
         kInvalidFreeCovNegSteppingHelper,
         kCovPos,
         kCovNeg,
         kCovPosSteppingHelper,
         kCovNegSteppingHelper,
         kCovPosCKF,
         kCovNegCKF,
         kInvalidCovPosCKF,
         kInvalidCovNegCKF,
         kUpdates,
         kUpdatesValidInput,
         kUpdatesValidInputAndOutput,
         kNCounter
      };

      ~Statistics();
      void boundCovChanged()              { ++(m_counter[kBoundCovChanged]); }
      void fullTransportJacobianChanged() { ++(m_counter[kFullTransportJacobianChanged]); }
      void freeToPathDeritivesChanged()   { ++(m_counter[kFreeToPathDeritivesChanged]); }
      void boundToFreeJacobianChanged()   { ++(m_counter[kBoundToFreeJacobianChanged]); }
      void freeTransportJacobianChanged() { ++(m_counter[kFreeTransportJacobianChanged]); }
      void sampling()                     { ++(m_counter[kSampling]); }
      void notSampling()                  { ++(m_counter[kNotSampling]); }
      void notSamplingSteppingHelper()    { ++(m_counter[kNotSamplingSteppingHelper]); }
      void samplingSteppingHelper()       { ++(m_counter[kSamplingSteppingHelper]); }
      void validCov(bool forward)       { ++(m_counter[forward ? kCovPos : kCovNeg]); }
      void validCovSteppingHelper(bool forward)       { ++(m_counter[forward ? kCovPosSteppingHelper : kCovNegSteppingHelper]); }
      void invalidCov(bool forward)       { ++(m_counter[forward ? kInvalidCovPos : kInvalidCovNeg]); }
      void invalidFreeCov(bool forward)       { ++(m_counter[forward ? kInvalidFreeCovPos : kInvalidFreeCovNeg]); }
      void invalidCovSteppingHelper(bool forward)       { ++(m_counter[forward ? kInvalidCovPosSteppingHelper : kInvalidCovNegSteppingHelper]); }
      void invalidFreeCovSteppingHelper(bool forward)       { ++(m_counter[forward ? kInvalidFreeCovPosSteppingHelper : kInvalidFreeCovNegSteppingHelper]); }
      void validCovCKF(bool forward)       { ++(m_counter[forward ? kCovPosCKF : kCovNegCKF]); }
      void invalidCovCKF(bool forward)       { ++(m_counter[forward ? kInvalidCovPosCKF : kInvalidCovNegCKF]); }
      void updateStatus(bool valid_input_cov, bool valid_output_cov) {
         ++m_counter[kUpdates];
         if (valid_input_cov) {
            ++m_counter[kUpdatesValidInput];
            if (valid_output_cov) {
               ++m_counter[kUpdatesValidInputAndOutput];
            }
         }
      }
      static Statistics &instance() {
         if (!s_instance) {
            ...
            if (!s_instance) {
               s_instance = std::make_unique<Statistics>();
            }
         }
         return *s_instance;
      }
   ...
   private:
      std::array<std::atomic<unsigned int>, kNCounter> m_counter {};
      static std::mutex s_mutex;
      static std::unique_ptr<Statistics> s_instance;
   };
...
public:
   ...
   const std::map <unsigned int, std::unordered_map<unsigned int, std::unordered_set< std::size_t> > > &
...
private:
   ...
   std::map <unsigned int, std::unordered_map<unsigned int, std::unordered_set< std::size_t> > > m_geoIds;
